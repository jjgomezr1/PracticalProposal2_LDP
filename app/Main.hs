...
guardarParqueadero parqueadero = do
    ...
    writeFileSeguro "parqueadero.txt" contenido
    putStrLn "Parqueadero guardado en el archivo parqueadero.txt."

-- Función para escribir en un archivo de forma segura
writeFileSeguro :: FilePath -> String -> IO ()
writeFileSeguro path contenido = catch (writeFile path contenido) manejarErrorEscritura

-- Función para manejar errores de escritura
manejarErrorEscritura :: IOException -> IO ()
manejarErrorEscritura e = putStrLn $ "Error al escribir en el archivo: " ++ show e
...
mostrarVehiculo vehiculo =
    "placa= " ++ placa vehiculo ++ ": Entrada= " ++ show (entrada vehiculo) ++ ", Salida= " ++ mostrarSalida (salida vehiculo)
    where
        mostrarSalida (Just s) = show s
        mostrarSalida Nothing = "Todavía en el parqueadero"

-- Función para mostrar la información de todos los vehículos en el parqueadero
mostrarParqueadero :: [Vehiculo] -> String
mostrarParqueadero parqueadero =
    "[" ++ intercalate ", " (map mostrarVehiculo parqueadero) ++ "]"

-- Función principal del programa
main :: IO ()
main = do
    -- Intentamos cargar el parqueadero desde el archivo de texto
    parqueadero <- catch (cargarParqueadero) manejarError
    putStrLn "¡Bienvenido al Sistema de Gestión de Parqueadero!"
    cicloPrincipal parqueadero

-- Función para manejar errores al cargar el parqueadero
manejarError :: IOException -> IO [Vehiculo]
...
cicloPrincipal parqueadero = do
    ...
    putStrLn "4. Listado de vehículos"
    putStrLn "5. Salir"
    ...
    case opcion of
        ...
        "2" -> do
            ...
            putStrLn $ "Vehículo con placa " ++ placaVehiculo ++ " salió del parqueadero."
            guardarParqueadero parqueaderoActualizado
            cicloPrincipal parqueaderoActualizado
        ...
        "4" -> do
            putStrLn (mostrarParqueadero parqueadero)
            cicloPrincipal parqueadero
